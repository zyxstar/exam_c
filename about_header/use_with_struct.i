...
TAG* map(TAG(*callback)(int), int *arr, int size);
# 4 "use_with_struct.c" 2
...
int main(){
  ...
  TAG *arr2 = map(add, arr, count);
  for(i = 0; i < count; i++ ){
    printf("%d %d;", arr2[i].add_2, arr2[i].multi_2);
  }
  free(arr2);
}
